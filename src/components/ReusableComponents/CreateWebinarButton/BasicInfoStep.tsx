...
const BasicInfoStep = (props: Props) => {
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='space-y-2'>
        <Label htmlFor='decription' className={errors.description ? 'text-red-400': ""}>
        ...
        <Textarea
          ...
          className={cn('min-h-[100px] !bg-background/50 border border-input' , errors.description && 'border-red-400 focus-visible:ring-red-400')}
        />
        {errors.description && (
        ...
        <div className='space-y-2'>
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant='outline' className={cn('w-full justify-start text-left font-normal !bg-background/50 border border-input' , !date && 'text-gray-500' , errors.date && 'border-red-400 focus-visible:ring-red-400')}>
                <Calendar1Icon className='mr-2 h-4 w-4' />
                {date ? format(date, 'PPP') : 'Select Date'}
              </Button>
            </PopoverTrigger>
            <PopoverContent>
              <Calendar mode='single' selected={date} onSelect={handleDateChange} initialFocus className='bg-background' 
              disabled={(date) => {
                const today = new Date()
                today.setHours(0,0,0,0)
                return date < today
          ...
          <div className='space-y-2'>
            ...
            <div className='flex gap-2'>
              ...
              <Select value={timeFormat || 'AM'} onValueChange={handleTimeFormatChange}>
      ...
      <div className='flex items-center gap-2 text-sm text-gray-400 mt-4'>
        ...
        <Button variant='outline' className='ml-auto relative border border-input hover:bg-background'>
          Upload File
