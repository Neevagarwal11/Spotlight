  ...
  return (
    ...
    <div className='space-y-6'>
        ...
        <div className='space-y-2'>
            ...
            className={cn('!bg-background/50 border border-input' , errors.ctaLabel && 'border-red-400 focus-visble:ring-red-400')}
            />
            {errors.ctaLabel && (
                ...
                <div className='flex flex-wrap gap-2 mt-2'>
                    {tags.map((tag:string , index:number) => (
                        <div key={index} className='flex items-center gap-1 bg-gray-800 text-white px-3 py-1 rounded-md'>
                            {tag}
                            <button onClick={() => removeTag(tag)} className='text-gray-400 hover:text-white'> <X className='h-3 w-3' /> </button>
                        </div>
                    ))}
...
{/* TODO: agents */}
        <div className='space-y-2'>
            ...
            <div className='relative'>
                <div className='mb-2'>
                    <div className='relative'>
                        <Search className='absolute left-3 top-2 h-4 text-gray-500' />
                        <Input placeholder='Search Agent' className='pl-10 !bg-background/50 border border-input ' />
                    </div>
                </div>
                    ...
                    <SelectContent className='bg-background border border-input max-h-48'>
                        {stripeProducts?.length > 0 ? (
                            stripeProducts.map((product) => (
                                <SelectItem 
                                    key={product.id}
                                    value={product?.default_price?.toString() || "no-price"}
